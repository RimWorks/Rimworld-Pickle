  ...
  private static async Task RunSmoke() {
    try {
      ...
      if (!found) {
        ...
        Log.ErrorTo(PickleLog.Channel, "widget capture smoke failed: btn:OK not found. known tags: {KnownTags}", [knownTags]);
        dialog.Close(false);
        return;
      }

      if (duplicate) {
        Log.ErrorTo(PickleLog.Channel, "widget capture smoke failed: btn:OK is ambiguous (duplicate)");
        dialog.Close(false);
        return;
      }
      ...
      try {
        ...
        Log.ErrorTo(PickleLog.Channel, clickEx, "widget capture smoke failed: ctx.Click(\"btn:OK\") threw");
        dialog.Close(false);
        return;
      }
      ...
      if (!closed) {
        Log.ErrorTo(PickleLog.Channel, "widget capture smoke failed: click did not close the dialog");
        dialog.Close(false);
        return;
      }

      Log.InfoTo(PickleLog.Channel, "widget capture smoke passed");
    } catch (Exception ex) {
      Log.ErrorTo(PickleLog.Channel, ex, "widget capture smoke failed with exception");
    }
  }
}
